...
exports.regNewEmployee = async (name, email, password, company_code) => {
    try {
        ...
        if (companyExist.rowCount < 1) {
            ...
            const newEmployee = await pool.query('insert into employees (name , email, password, company, company_id ) values ($1,$2,$3,$4,$5) returning *',[name,email,hashPassword, companyExist.rows[0].name,companyExist.rows[0].id]);
            ...
            if(newEmployee.rowCount<1){
                ...
                const { password , department,role,company_id,is_active, ...EmployeeDetails}=newEmployee.rows[0]
                return {
